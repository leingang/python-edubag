...
class TestGradescopeClient:
    ...
    def test_save_roster_with_course_id(self):
        ...
        with patch("pathlib.Path.exists", return_value=True):
            # Mock the session method
            with patch.object(
                client, "_save_roster_session", return_value=Path("roster.csv")
            ) as mock_session:
    ...
    def test_save_roster_with_course_url(self):
        ...
        with patch("pathlib.Path.exists", return_value=True):
            ...
            with patch.object(
                ...
                result = client.save_roster(
                    course="https://gradescope.com/courses/12345", headless=True
                )

                # Verify the session method was called with the full URL
                mock_session.assert_called_once_with(
                    "https://gradescope.com/courses/12345", None, True
                )
                assert result == Path("roster.csv")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("pathlib.Path.exists", return_value=True):
                ...
                with patch.object(
                    ...
                    result = client.save_roster(
                        course="12345", save_dir=save_dir, headless=True
                    )

...
def build_attendance_gradebook(
	...
	output: Annotated[
		Optional[Path],
		typer.Option(help="Output Brightspace gradebook CSV path. If omitted, writes to data/processed/brightspace/grades/<today>/attendance.csv"),
	] = None,
...
def build_engagement_gradebook(
	...
	categories_to_analyze: Annotated[
		Optional[str],
		typer.Option(
			help="Comma-separated list of grade categories to analyze (e.g., 'Pre-Quizzes,Pre-Surveys,Polls'). If not specified, will auto-detect all categories.",
		),
	] = None,
):
...
def aggregate_from_config(
	...
	for name, source_config in cfg['data_sources'].items():
		...
		if name == 'gradebook':
			...
			drop_cols = [
				c for c in source.grades.columns
	...
	for denom_name, denom_config in cfg.get('denominators', {}).items():
		...
		logger.debug(f"  Columns in merged_df: {[c for c in merged_df.columns if 'exemptions' in c or 'denominator' in c]}")
